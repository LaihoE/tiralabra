...
class Huffman:
    def __init__(self, codelengths):
        """
        Generates huffman trees from codes read earlier in 
        """
        self.bits_to_symbol = {}
...
def generate_huffman_trees(bitreader):
    """
    Generates literal code trees and distance code trees.
    """
    n_literal_codes = bitreader.read_n_bit_int(5) + 257
    n_distcodes = bitreader.read_n_bit_int(5) + 1
    codelen_arr = generate_codelen_arr(bitreader)
    
    code_lengths = Huffman(codelen_arr)
    codes = generate_codelengths(code_lengths, n_literal_codes, n_distcodes, bitreader)
